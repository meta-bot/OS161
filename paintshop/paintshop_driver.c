...
void customer(void *unusedpointer, unsigned long customernum)
{
  struct paintcan can;
  int i,j;
  int check_if_doubled[NCOLOURS+3];
...
#endif
    for(j=0;j<NCOLOURS+3;j++)check_if_doubled[j] = 0;
    /* erase ingredients list on can and select a colour in terms of tints */
    for (j = 0; j < PAINT_COMPLEXITY; j++) {
    int choosen = 0;  
    while(1){
	choosen = (int)(random()%(NCOLOURS+1));
	if(choosen<0 || choosen >NCOLOURS)continue;
	if(check_if_doubled[choosen] == 0)break;
      }
      can.requested_colours[j] = choosen;
      check_if_doubled[choosen] = 1;
    }
    //can.requested_colours[0] = RED;
    //can.requested_colours[1] = 0;
